...
var minDominoRotations = function (tops, bottoms) {
  const check = (target) => {
    ...
    for (let i = 0; i < tops.length; i++) {
      ...
      // If top is not the target, we need to rotate to bring target to top
      if (tops[i] !== target) rotationsTop++;
      // If bottom is not the target, we need to rotate to bring target to bottom
      if (bottoms[i] !== target) rotationsBottom++;
    }

    // Return the minimum rotations needed (either top or bottom)
    return Math.min(rotationsTop, rotationsBottom);
  };

  // Try both tops[0] and bottoms[0] as possible uniform values
